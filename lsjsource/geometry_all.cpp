...
db nearest(P p1,P p2,P q){
    ...
    if(isMiddle(p1,h,p2))
        return q.dist(h);
    return min(p1.dist(q),p2.dist(q));
}
...
db area(vector<P> ps){
    db ret = 0; forn(i, sz(ps)) ret += ps[i].det(ps[(i+1)%ps.size()]); 
    return ret/2;
}
...
db convexDiameter(vector<P> ps){
    ...
    int i = is, j = js;
    db ret = ps[i].dist(ps[j]);
    do{
        if((ps[(i+1)%n]-ps[i]).det(ps[(j+1)%n]-ps[j]) >= 0)
            (++j)%=n;
        else
            (++i)%=n;
        ret = max(ret,ps[i].dist(ps[j]));
    }while(i!=is || j!=js);
    return ret;
}
...
db min_dist(vector<P>&ps,int l,int r){
    if(r-l<=5){
        ...
        for (int i=l;i<r;++i) for(int j=l; j<i;++j)
            ret = min(ret,ps[i].dist(ps[j]));
        return ret;
    }
    int m = (l+r)>>1;
    db ret = min(min_dist(ps,l,m),min_dist(ps,m,r));
    vector<P> qs; for(int i=l;i<r;++i) if(abs(ps[i].x-ps[m].x)<= ret) qs.push_back(ps[i]);
    sort(qs.begin(), qs.end(),[](P a,P b) -> bool {return a.y<b.y; });
    for(int i=1;i<sz(qs);++i) for(int j=i-1;j>=0&&qs[j].y>=qs[i].y-ret;--j)
        ret = min(ret,qs[i].dist(qs[j]));
    return ret;
}
...
vector<P> isCL(P o,db r,P p1,P p2){
    if (cmp(abs((o-p1).det(p2-p1)/p1.dist(p2)),r)>0) return {};
...
vector<L> extanCC(P o1, db r1, P o2, db r2) {
    ...
    if (cmp(r1, r2) == 0) {
        ...
        forn(i,min(sz(ps),sz(qs))) ret.push_back(L(ps[i], qs[i])); //c1 counter-clock wise
    }
    return ret;
}
...
P circumCenter(P a, P b, P c) { 
    P bb = b - a, cc = c - a;
    db db = bb.abs2(), dc = cc.abs2(), d = 2 * bb.det(cc);
